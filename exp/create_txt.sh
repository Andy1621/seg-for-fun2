...
# 开始生成数据
# cd $target_dataset_path
# if [ ! -f $target_dataset_path/train_data/img_train ] || [ ! -f $target_dataset_path/train_data/lab_train ]; then
#     echo `pwd`
#     cd $original_dataset_path
...
cd $current_path
if  [ ! -d $target_dataset_path/train_data ]; then
    echo "create training dataset directory..."
    mkdir $target_dataset_path/train_data
fi

cd $original_dataset_path
if [ -f img_train.zip ] && [ -f lab_train.zip ]; then
    if [ -f $target_dataset_path/train_data/train_list.txt ]; then
        echo "File train_list.txt has existed."
    elif [ -f $target_dataset_path/train_data/val_list.txt ]; then
